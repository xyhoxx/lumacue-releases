...
บันทึกการเปลี่ยนแปลงภาษาไทยของ LumaCue ครอบคลุมทุกรุ่นที่เผยแพร่สู่สาธารณะ ตั้งแต่ `0.1.1` ถึงรุ่นปัจจุบัน ดูรายละเอียดเชิงเทคนิคภาษาอังกฤษได้ที่ [CHANGELOG_EN.md](CHANGELOG_EN.md)

**ภาษา:** [ไทย](CHANGELOG.md) | [English](CHANGELOG_EN.md)

## 0.8.5 - Hotfix: ชื่อเพลงแรกหลังเปิดโปรแกรม

### แก้ไข
- แก้ต้นเหตุที่ชื่อเพลงแรกซึ่งกู้คืนจากคิวหลังเปิดโปรแกรมยังถูกการซิงก์ภาษาเขียนทับเป็น `Waiting for a song...`
- แยกชื่อเพลงที่เปลี่ยนตามการเล่นออกจากข้อความคงที่ของระบบแปลภาษา โดยยังแปลข้อความรอเพลงตามภาษาที่เลือกได้ตามเดิม

## 0.8.4 - Hotfix: การเริ่มเล่นเพลง, Overlay และขั้นตอนปล่อยเวอร์ชัน

### แก้ไข
- แก้ชื่อเพลงแรกหลังเปิดโปรแกรมที่บางครั้งยังแสดงเป็น `Waiting for a song...` ทั้งที่เริ่มเล่นแล้ว
- ให้ Overlay แสดงค้างไว้เป็นค่าเริ่มต้นแทนการหุบเอง พร้อมย้ายค่าของผู้ใช้เดิมไปยังค่าใหม่เพียงครั้งเดียว โดยยังเลือกให้หุบอัตโนมัติได้
- ให้ตัวเลือกในหน้า Overlay Settings ที่ยังใช้ไม่ได้ตามค่าปัจจุบันแสดงอยู่ในสถานะปิดใช้งานแทนการซ่อนหายไป
- แก้มุมแหลมรอบเงาของ native launcher โดยเว้นพื้นที่ให้เงาอยู่ภายในขอบหน้าต่าง
- เปลี่ยนการเลือกเพลงชื่อซ้ำจาก Twitch ให้ผู้ชมพิมพ์ชื่อศิลปินโดยตรง และค้นหาศิลปินเพิ่มเติมได้เมื่อไม่มีชื่อที่ต้องการในตัวเลือกแรก
- ให้ backend ใช้พอร์ตที่ desktop กำหนดได้ครบทั้งฝั่ง Python และ Player iframe ลดโอกาสเปิดโปรแกรมไม่ได้เมื่อพอร์ตเดิมไม่ว่าง

### ปรับปรุง
- เพิ่ม release gate ใน CI/CD ให้รัน Python, renderer, launcher, Discord, updater และ native launcher tests ก่อนสร้างไฟล์ปล่อย
- เพิ่ม packaged smoke test ที่เปิดแอปจาก `win-unpacked` จริง ตรวจ backend, Player iframe, Library และการปิด process หลังจบทดสอบ
- อัปเดต `electron-updater`, `electron-builder` และ dependency ที่เกี่ยวข้องเป็นรุ่นที่แก้ช่องโหว่แล้ว โดย production dependency audit ไม่พบช่องโหว่ที่ทราบในขณะปล่อยรุ่นนี้
...
### แก้ไข
- Auto DJ ใช้เพลงที่กำลังเล่นและเพลงถัดไปในคิวเป็นต้นทางหลักสำหรับหาเพลงที่เกี่ยวข้องหรือเพลงของศิลปินเดียวกัน แทนการเริ่มจากประวัติการเล่น
- ค่าเริ่มต้นของ Auto DJ จะไม่หยิบเพลงเก่าจากประวัติการเล่นกลับมาเข้าคิวอีก โดยการตั้งค่า history ที่ผู้ใช้กำหนดเองยังใช้งานได้

### เปลี่ยนแปลง
- เริ่มย้าย launcher จาก Electron ไปเป็น native Windows launcher ขนาดเล็ก โดย launcher เดิมสามารถอัปเดตตัวเองไปเป็นรุ่นใหม่ได้ผ่าน manifest เดิม
- เมื่อ native launcher เข้ารับงานแล้ว จะลบเฉพาะไฟล์ Electron bootstrap เก่าที่ไม่ใช้แล้ว รวมถึง resources, locales และ Chromium runtime files พร้อมเก็บเฉพาะเวอร์ชันแอปปัจจุบัน, runtime ที่ใช้ร่วมกัน และข้อมูลผู้ใช้
- คงการทำงานของการถอนการติดตั้ง, การตรวจ SHA-512, patch/app-only/runtime updates และ offline fallback ไว้เหมือนเดิม
